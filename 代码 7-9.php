...
echo "按照Microsoft Word中字数统计的方法，这句话里的字数有： ";  
echo mb_strlen($chinese, "utf8")+str_word_count($text)+count($matches[0])+count($punct_matches[0])+count($chinese_punct_matches[0])." 个";  
echo "按照Microsoft Word中字数统计的方法，这句话里的字数有： ";  
echo mb_strlen($chinese, "utf8")+str_word_count($text)+count($matches[0])+count($punct_matches[0])+count($chinese_punct_matches[0])." 个"."<br>";  
  
echo "按照Microsoft Word中字数统计的方法，这句话里的字符数（不计空格）有： ";  
echo mb_strlen($chinese, "utf8")+$english_char_num+$number_char_num+count($punct_matches[0])+count($chinese_punct_matches[0])." 个"."<br>";  
  
echo "按照Microsoft Word中字数统计的方法，这句话里的字符数（计空格）有： ";  
echo mb_strlen($chinese, "utf8")+$english_char_num+$number_char_num+count($punct_matches[0])+count($chinese_punct_matches[0])+substr_count($text," ")." 个"; 
?>  
